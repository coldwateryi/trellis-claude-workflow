...
1. 已设置的 `TRELLIS_WORKFLOW_INSTALL_DIR`
2. 从当前目录向上查找最近的 Trellis 初始化项目，并安装到该项目的 `.claude/workflows`
3. 通过 `--project-dir` / `TRELLIS_WORKFLOW_PROJECT_DIR` 指定 Trellis 初始化项目，并安装到该项目的 `.claude/workflows`
4. 只有传入 `--global` 或交互确认时，才安装到全局 `~/.claude/workflows`

Trellis 初始化项目的识别规则与 trellis-skills 安装器一致：项目根目录存在 `.trellis/`。

同一个安装脚本同时支持本地 clone 安装和完全在线安装。从 git clone 项目目录运行时，脚本直接使用本地 workflow 文件；通过 GitHub raw 下载运行时，脚本会自动下载仓库 archive 后安装。
...
如果从 Trellis 初始化项目内部运行，`--project-dir` 可省略，脚本会优先安装到当前项目级。需要安装到全局时：
